...
drop policy if exists "Users can read their own workout images" on storage.objects;
create policy "Users can read their own workout images"
on storage.objects
for select
to authenticated
using (
...
drop policy if exists "Users can update their own workout images" on storage.objects;
